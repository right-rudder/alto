const commercialPilotDataGroundSchool = {
  data: {
    siteTitle: "Commercial Pilot Ground School | Flight School Oklahoma City",
    siteDescription: "Obtaining your commercial pilot certificate allows you to fly for compensation or hire.  With those new privileges, you will be required to learn additional regulations, become more familiar with how weather works, and learn about more complex aircraft. Start with taking our commercial pilot ground school located in Sundance Airpark over in Oklahoma City.",
    date: "Commercial Pilot Ground School",
    hiddenField: "Commercial Pilot Ground School",
    title: "Pass your FAA commercial written exam",
    price: "$1350",
    intro: [
      "If you are an instrument rated private pilot, getting your commercial pilot certificate is the next part of your aviation journey. Obtaining your commercial certificate allows you to fly for compensation or hire. To fly safely and professionally, you'll need to become familiar with advanced flight maneuvers, commercial regulations, and how to operate complex aircraft. ",
      "Flying as a commercial pilot means adhering to higher standards of precision and proficiency. You'll need to master flight maneuvers, navigate challenging situations, and communicate effectively with ATC (air traffic control). Additionally, while having a commercial certificate opens up many opportunities, you'll need to understand the limitations and regulations to ensure safe and efficient operations. Enrolling in our commercial ground school is your chance to meet with a CFI and other pilots who are in similar stages of their aviation careers. Give us a call today and stop by Alto Flight Academy for more information."
    ],
    content: [
      "Our Commercial Pilot Ground School are group classes held at Sundance Airport KHSD in Oklahoma City, OK. These classes will help prepare you to take the FAA written exam: one of the prerequisites you must complete in order to get your commercial pilot certificate. When you enroll in our commercial ground school course, you'll also get your ground school kit. If you already purchased a ground school kit for instrument ground school, then you don't need to purchase the kit",
      
      "This kit contains all of the essentials you'll need to start your commercial pilot ground training. You'll receive a syllabus which contains the lesson plans of your future flights and also the specific ground lessons we'll go over. You'll also receive the Jeppesen Instrument and Commercial textbook which we will use in the course. To enroll in our Commercial Pilot Ground School, it is $645 to enroll and $354.96 for the ground school kit. At the end of the course, you'll receive an endorsement from our certified flight instructor.",
    ],
    headerImage: "/src/assets/alto-aircraft-airplane-fleet-on-grass-runway-airport.jpg",
    headerAlt: "Alto Flight Academy fleet of aircraft on grass airport Cedar Mills",
    primaryImage:
      "/src/assets/commercial-pilot-passing-exam-in-bethany-oklahoma.jpg",
    primaryAlt: "Commercial pilot passing FAA practical exam in Bethany Oklahoma",
    secondaryImage: "/src/assets/female-pilot-in-front-of-a-king-air-airplane.jpg",
    secondaryAlt: "Female pilot in front of a king air airplane",
    prerequisites: [
      "Meet your instructor and classmates",
      "Aviation opportunities and career pathways",
      "Advanced navigation and flight planning",
      "Commercial aviation weather analysis",
      "Commercial regulations and air traffic control",
      "Aircraft systems and performance",
      "Advanced human factors and safety",
      "Commercial flight maneuvers and techniques",
      "Complex aircraft operations",
      "Emergency procedures and decision making",
      "Understanding commercial pilot limitations and responsibilities",
      "Preparation for the FAA commercial pilot practical exam"
    ],
    process: [
      "Get in contact with us",
      "Learn more about our ground school",
      "Enroll into the next start date",
      "Attend ground school and participate",
      "Complete the final exam",
      "Receive endorsement from a qualified instructor",
      "Complete the FAA written exam",
    ],
  },
  slug: "commercial-pilot",
};

export default commercialPilotDataGroundSchool;
